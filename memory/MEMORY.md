...
- [Project: LC build progress](project_lc_build_progress.md) — refreshed 2026-06-09: Steps 1–6 complete; **Step 7 Slices 1, 4a, and 4b shipped** (walking skeleton → edit/relocate/delete → sequence insert/reorder + returning/intra-metro detection); what's running; build decisions + bugs fixed. Read first when resuming build work.
- [Decision: Step 7 phased into build slices (2026-06-05)](decision_step7_slice_phasing_2026-06-05.md) — **Read first for Step 7.** Walking-skeleton phasing (Slices 1–5). **Slices 1 + 4a + 4b SHIPPED**; **Slice 2 (richness: image + AI extraction) or Slice 3 (place types) is next.** Records the design calls (modal-first / globe projection / arc-drag-insert deleted / image in Slice 2 / Timeline separate), the self-entity decision, satellite-zoom enhancement, and the Raw-Vault/edit-capability gap.
