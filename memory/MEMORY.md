...
- [Project: LC build progress](project_lc_build_progress.md) — **refreshed 2026-07-05** (top "Session handoff" block = current state); Steps 1–6 complete; **Step 7 Slices 1, 2, 4a, 4b + the globe-legibility track (3 close-out, 3.5 reworked, 3.6 "Log" pin) shipped; Slice 6 (Entity View + context) build COMPLETE incl. 6.5b (orchestrator context auto-proposal, 2026-07-05) — Andy's slice QA walkthrough pending; Resume View next**. What's running; build decisions + bugs fixed. Read first when resuming build work.
