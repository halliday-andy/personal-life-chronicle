...
**Orchestrator Agent layer (added 2026-05-17):** A new architectural layer was added with the capture-assistant work — the Orchestrator Agent. It sits between user submissions and the existing sub-agents (Tagger, Entity, future Temporal). The orchestrator (Claude Sonnet 4.5) receives every submission with broad chronicle context, reasons aloud about placement, and delegates to sub-agents via tool use. Sub-agents are dual-mode: Inngest async listeners (deeper passes after `memory/ingested`) and inline-callable tools (synchronous, used by the orchestrator for the immediate response). The orchestrator's prompt has three layers — generic system prompt (user-agnostic, version-controlled), per-user chronicle context digest (cached via Anthropic prompt caching, regenerated by a Planner-owned compaction job), and submission-time inputs (fresh each call). This makes the system multi-tenant safe by construction and cost-efficient at scale. See `documentation/feature_capture_assistant.md` v1.1 and `memory/project_lc_capture_assistant.md`.

**Second content layer — private notes (added 2026-05-17):** Every memory has two content layers: public content (governed by Access Cards) and `memories.private_notes` (owner-only, filtered out of non-owner projections via RLS column-level filtering regardless of which Access Cards the memory is granted to). This is a content-layer split, not an additional card tier. Use case: a memory shared via a Family card may still contain things the user wants kept private (honest assessments, social-context reminders). The schema delta is a single column on `memories`; the access enforcement is in `viewer_can_access()` (when fully implemented in Step 13) which projects non-`private_notes` columns for non-owners.
