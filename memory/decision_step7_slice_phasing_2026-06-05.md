...
- **Slice 3 — place types: DESIGNED 2026-06-12, canonical spec now `docs/plans/2026-06-12-globe-place-types-design.md`.** Resolved with Andy into **six** types (not four): Primary residence (`lived_at`), Workplace (`worked_at`), Second residence (`owned_residence_at` — "a second home you returned to": owned/fractional/share-house/repeat-rental, not ownership-defined), Short-term stay (`lived_briefly_at`), Vacation (`vacationed_at`, NEW), Professional travel (`traveled_for_work_to`, NEW). **Model A:** only primary residences form the connected glowing-chevron spine (the temporal scaffold); the other five are time-anchored markers carrying `relationships.anchor_residence_id` (NEW nullable col, ON DELETE SET NULL) chosen by the user at placement. **Three line tiers:** spine (solid glow chevron) ▸ commute line home→workplace (superior, own iconography) ▸ dashed trip tethers. Time marker = existing `when_text` chip per pin (era-coloration + proportional timeline deferred to Temporal Agent). **Deferred vision:** a toggle/filter promoting any type into its own central transit (e.g. professional through-line). Migration `20260613130000_globe_place_types.sql` + 4 RPC rewrites; phases tracked as tasks #4–#7. (Earlier "vacation = owned vs visited" question resolved: owned-second-home → Second residence; visited-leisure → Vacation.)
...
- **Arc directionality — SHIPPED EARLY 2026-06-11** (overnight autonomous session, commit `699346f`) rather than waiting for Slice 3: per-leg arc segments, faint resting chevrons, inbound-brighter/outbound-softer emphasis on selection. Slice 3 still owns integrating this with per-place-type arc styling. Original decision record: **(Andy, 2026-06-10, agreed in principle):** the residence arcs currently show sequence with no direction. Agreed treatment: contextual direction on pin selection — the selected pin's inbound arc reads "approached from" and outbound "egressed to" (chevron at the pin's edge and/or brighter-inbound/softer-outbound gradient); optionally a subtle ember-flow animation along arcs globally. Deliberately NOT arrowheads on every segment at rest — keeps the zoomed-out nocturne globe calm. Slice 3 owns per-type pin/arc styling, so directionality lands there.
- **Timelines surface — Andy's dimension examples (2026-06-10):** when the Timelines surface (PRD v1.1 third navigation surface) is specced, Andy explicitly wants per-entity-type and per-dimension temporal progressions — e.g. *vehicles owned* (entity type already in schema) and *operating principles/values that drove different life phases* (values dimension + wisdom_distillation). The residential spine being built now is the temporal scaffold these hang off (invariant #5).
- **Multi-photo gallery per pin — SHIPPED 2026-06-13** (overnight, commit `811851c`): edit-panel gallery with add/make-primary/remove; card photo = the primary; Replace demotes instead of deleting. Client HEIC→JPEG preprocessing applies to gallery adds too. Original record: **(Andy, 2026-06-10, after first Slice 2 use):** the edit panel should eventually allow attaching *multiple* photos to a residence, while the pin/detail-card photo stays single — it's whichever is flagged primary. Schema already supports this (`entity_media` many-per-entity + `is_primary`); the one-image limit lives only in `attachPinImage`'s replace-on-attach behaviour, so the change is additive: append non-primary rows, add a gallery + "make primary" UI in the edit panel. Pairs with the also-deferred image preprocessing (client-side HEIC→JPEG conversion + ~2MB compression — HEIC uploads work today but only render in Safari).
