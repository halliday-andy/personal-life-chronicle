...
**Residential history as Phase 0 (keystone insight, April 2026; UI revised May 2026):** The sequence of homes a person has lived in is the single most powerful temporal scaffold because of strict sequential non-overlap — one primary home at a time. Each confirmed move date generates TWO constraints simultaneously: upper bound on the previous home period, lower bound on the next. The residential spine is built FIRST before any other temporal resolution work. `generate_residency_constraints()` auto-fires when a lived_at relationship is added/updated.

**UI mechanism (2026-05-17 revision):** The residential spine is built via direct globe pin placement, not text interview. The user pins places on a Mapbox globe in life order; each click creates a `place` entity + `lived_at` relationship; `sort_order` captures sequence even when dates are unknown. Drag-arc-to-insert handles forgotten intermediate residences and order corrections. Side trips (`lived_briefly_at`) and vacation homes (`owned_residence_at`) are layered on with distinct visual styling. The capture assistant runs as a sidekick chat alongside the globe and captures memories anchored to each pin. See `documentation/feature_residential_globe_onboarding.md` v1.1 for the full UX. The constraint graph mechanism described above remains unchanged — what changed is the input modality (clicks + modal text instead of conversational Q&A).
