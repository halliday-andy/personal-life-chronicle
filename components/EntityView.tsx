        ...
        <section className="mt-6">
          ...
          {adding && (
            <div className="mt-3 rounded-xl border border-stone-200 bg-white p-4">
              <textarea
                ...
                placeholder="Background, research, a fact worth keeping…"
                rows={4}
          ...
          {shareable.length > 0 && (
            <div className="mt-3 space-y-2">
              {shareable.map((n) => <NoteCard key={n.id} note={n} onRemove={() => removeNote(n.id)} />)}
            </div>
          )}
          ...
          {priv.length > 0 && (
            <div className="mt-4">
              ...
              <div className="space-y-2 rounded-xl border border-amber-200 bg-amber-50/60 p-2">
                {priv.map((n) => <NoteCard key={n.id} note={n} onRemove={() => removeNote(n.id)} />)}
              </div>
            </div>
          )}
        </section>
