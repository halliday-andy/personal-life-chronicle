...
### 4.5 Catching near-duplicate entities in the moment (added 2026-05-22)

The orchestrator's Layer B context already lists the user's known persons, places, and organisations by canonical_name + aliases. When a new submission introduces a name that's *close to but not identical to* something in Layer B (a typo like "Leola Lapidus" when "Leola Lapides" already exists), the orchestrator should catch the discrepancy immediately, not let the Entity sub-agent create a duplicate that the user resolves later from the Review Queue.

**Why this matters:** post-hoc cleanup via Review Queue is a backlog model — every typo accumulates as a pending decision the user has to revisit. In-the-moment catching uses the highest-context moment (the user is engaged with this exact piece of content, right now) to surface the discrepancy when the cost of resolution is lowest. The Review Queue remains the backstop for things the orchestrator missed — it doesn't go away, it shrinks.

**Behaviour:** before calling `extract_entities`, the orchestrator scans Layer B for likely matches to names in the submission. When it finds a near-match (one to a few characters off; shared first or last name; phonetically close), it does one of three things based on confidence:

| Confidence | Behaviour | Example |
|---|---|---|
| Very high (e.g., 1-char typo) | Auto-link to canonical, surface "I corrected this — undo?" in the reasoning narrative | "Leola Lapidus" → silently linked to existing "Leola Lapides" with a visible note |
| High but ambiguous | Surface a two-option proposal card: "Link to existing X" (default) vs "Create new entity Y" — user clicks once | "Bob Jenkins" when "Bob Jenkin" exists — could be a typo or a different person |
| Lower | Ask in the reply text: "I noticed you wrote X — is this the same person as Y who you've mentioned before?" — user's next message resolves | "Jen" when "Jenny" exists — could be a nickname or a different person entirely |

**Implementation hooks:**
- The orchestrator system prompt (`lib/agents/orchestrator/system.ts`) carries a directive describing the scan-and-flag behaviour
- A new tool `link_to_existing_entity(extracted_name, target_entity_id, memory_id)` lets the orchestrator bypass the Entity Agent's fuzzy resolution when it has already resolved the entity confidently from Layer B context
- The proposal card UI (substep 6f) supports the two-option pattern

**Relationship to the Entity Agent's matching algorithm:** complementary, not redundant. The orchestrator is the smart frontline using LLM judgment over Layer B context. The Entity Agent's algorithm is the rigid backstop for non-orchestrator paths (future bulk imports, source-document ingestion, etc.). Both improve resilience.

### 4.6 Prompt caching and chronicle context compaction
...
| OQ-CA-4 (chronicle context size) | ✅ Use the dream-compaction pattern (per §4.6). Background "context digester" produces a compact 1–3k-token per-user digest, stored separately, marked for prompt caching. Design for prompt caching from day one so the system prompt + digest aren't re-delivered on every call. |
