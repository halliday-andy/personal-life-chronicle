-- 2026-06-11 — Scope pin edit/delete to the pin's OWN globe memory.
--
-- update_residence_pin resolved "the pin's memory" with an unordered
-- LIMIT 1 over every role='location' link, and delete_residence_pin
-- deleted EVERY memory linked to the place. Both are wrong as soon as
-- a second memory references the same place (e.g. a capture-assistant
-- recollection that mentions Lockbourne AFB): edits could target — and
-- pin deletion would destroy — memories that don't belong to the pin.
--
-- Both functions now scope to capture_mode='globe_onboarding' (the
-- modal/panel-authored recollection), oldest-first for determinism.
-- Non-globe memories that mention the place are never touched; on pin
-- deletion they simply lose their memory_entities link if the place
-- entity itself is removed (FK cascade), but the memories survive.
--
-- CREATE OR REPLACE FUNCTION only — no existing data altered.
...
BEGIN
    ...
    -- Only the pin's own globe-authored recollection dies with the pin.
    -- Other memories that mention this place are not the pin's property.
