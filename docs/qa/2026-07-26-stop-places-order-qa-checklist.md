...
- [ ] Open **My Mt. Snow Chalet** → the detail card shows **"Places at this
...
- [ ] There is **no longer** an "N related pins" chip — recollections,
      context and jots keep theirs.
- [ ] The **edit panel** shows the same block, same order.
- [ ] Drag to reorder there → persists, and matches Journey after a reload.
- [ ] Open a **workplace** with Logs under it (Coronet Peak Ski School) → the
      heading reads **"Related places"**, not "Places at this stop" (a
      workplace isn't a stop).
- [ ] A pin with no anchored places shows no block at all.
...
- [ ] Reorder, then add a new place anchored to that stop → the newcomer
      appears at the **end**, and nothing you arranged moved.
- [ ] Reorder in two browser tabs on the same stop → the second save wins and
      **no place disappears** from either.
- [ ] With the dev server stopped, drag → an error appears and the order is
      **put back**, not left half-applied.

## 4. Regression

- [ ] Clicking a place in the block still selects/opens that pin.
- [ ] Spine order, stop numbers, the "… more" recollection expansion and the
      facts editor are all unaffected.
- [ ] `node scripts/verify-stop-order.mjs` passes (19/19).
