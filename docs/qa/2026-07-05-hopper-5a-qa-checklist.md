...
- [x] Click any pin → the count-chip row now always shows a **✎ jot** chip
      (even on a pin with no recollections/context). **[taste]** the chip label
      ("jot" vs alternatives) and its always-present-ness.
- [x] Tap it → a **"Jot a memory to come back to…"** input appears. Type a
      fragment (e.g. "the ice-cream truck summer") → **Enter** (or Jot) → it
      appears in the list instantly; the chip now reads **✎ 1 to write**.
- [x] Add two or three more in quick succession — the input stays focused so
      you can jot at thinking speed.
- [x] Switch to another pin and back → the jots persist; the chip count is
      right on arrival (no flicker to "jot").

## 2. The full hopper on the edit panel
- [x] Open the same pin → **Edit** → below the photo gallery there's a
      **"Memories to write — jot now, recollect later"** section listing the
      open jots with checkboxes. **[taste]** the section label.
- [x] **Check one off** → it moves into a collapsed **"N written"** group
      (strikethrough, ✓).
- [x] Expand "N written" → **↩ reopen** puts it back on the open list;
      **✕** removes it permanently.
- [x] Hover an open jot → **✕** appears; removing it deletes without ceremony
      (it's a to-do, not an audit trail).

## 3. Cross-surface consistency
- [x] Check a jot off on the edit panel → close the panel → the detail-card
      chip count dropped to match.
- [x] A pin with zero open jots shows the invitational empty line ("Nothing
      waiting — jot the memories this place brings to mind…"), not a bare box.

## 4. Not in the way
- [x] The Journey/globe reading surfaces are unaffected — the hopper appears
      ONLY on the pin detail card (behind its chip) and the edit panel
      (per the brief: compact sequence surfaces stay clean).
- [x] The chip row doesn't push the card over its own pin awkwardly on a pin
