...
- **Approach:** Per KTD10: the sequence-position picker (the "Before X (earliest)" / "After X" dropdown, in both PinModal and PinEditPanel) gains a "Decide later — not yet placed" option. At creation it writes `lived_at` with `sort_order NULL` (no insert-and-shift); on an already-sequenced pin it demotes it out of the spine, resequencing the remainder (the re-type-away-from-Primary path already does this). "Place in sequence →" on the detail/edit panel reopens the picker and runs the existing insert-and-shift. Distinct not-yet-placed pin treatment sharing the trip-draft badge language. Wire U3's "add an origin pin" to create one and set it as trip origin. Audit spine-derived paths to ignore NULLs: `reorder_residence_pins` rejects unsequenced ids; Journey shows unsequenced residences in a "not yet placed" group, never as spine stops; origin-star and present-tense "now" logic key on ordered pins only; `nearest_residence` stays scoped to sequenced primaries.
...
- **Test scenarios (verify extension + QA checklist):**
  ...
  - Demote a sequenced pin to "decide later" from the edit panel → it leaves the thread, the remaining spine resequences contiguously, and the pin keeps its recollection, photos, facts, and jots; re-placing it restores it at the chosen position.
