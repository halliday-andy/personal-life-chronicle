...
| **F19** | Hovering a pin peeked its tethers but never its **trips** — the tether effect honoured `hoverPreview`, the route effect only honoured `selectedId`. **The REPORTED case was not this bug:** no trip touches Coronet Peak Ski School (verified — 0 rows), so that pin correctly showed nothing; the trip belongs to the similarly-named *residence* metres away. The asymmetry is real and was fixed, but it was never what Andy hit | hover counts alongside selection for routes; drafts still draw nothing at rest. **Test by hovering "Coronet Peak, Queenstown NZ", the actual origin** | **R16 — BUILT, untested against a real case** |
| **F20** | The chronicle's line tiers are coloured for NOCTURNE and hardcoded at layer creation, so on the daylight basemap a Log's dashed tether all but vanishes. The 2026-07-18 regime swaps the canvas; nothing re-tuned what is drawn on it | `chronicleLinePaint(regime)` — darker hues, higher opacity, **no blur** on light (blur reads as glow on dark, smudge on light). Identity preserved, only values change | **R17 — BUILT** |
| **F21** | A selected pin painted its trip routes **even with routes toggled OFF**, with no way to stop it — the legend could only add. R10's "always show the selected pin's trips" assumed one or two; a home with many departures buries the map. R16 widened it further by adding hover | the card's ✈ chip gates the routes: opening it shows, closing it hides. Hover stays a transient peek | **R18 — BUILT** |
| **F22** | When a trip destination is anchored to that trip's ORIGIN, the dashed tether and the solid route describe the **same segment** — two systems drawing one relationship, stacked. Confirmed: Sheep Station's anchor IS Coronet Peak, Queenstown NZ, the trip's origin | suppress the tether **only while that pair's route is actually drawn** — unconditional suppression would lose the only "belongs to that home" cue when routes are hidden. Needs the two line effects to coordinate | **DEFERRED to the globe visual-language pass** |
| **F23** | The detail card could grow to ~85% of the viewport (`max-h-[calc(100vh-8rem)]`) in the middle of the map, so opening the trips chip meant losing the globe you were exploring | cap at **55vh** (it has scrolled internally since R8) + `.globe-scroll`, an ember-tinted scrollbar, since with a cap the bar becomes the "more below" affordance | **R19 — BUILT** |
| **F24** | Clicking a trip's destination required a further click to reveal the trip and its route | a trip's **DESTINATION** auto-opens its trips panel, which via R18's gate also paints the route. **Not origins** — a home with many departures is exactly what R18 stopped from burying the map | **R19 — BUILT** |
| **F25** | The globe's **draft confirm bar** ("Drag the pin to the exact spot") had no Escape — R1 covered `PinModal` and `TripFramePanel` and missed a third surface of the same kind. The R1 checklist also mis-described it as "the pin placement modal", sending Andy to the wrong surface | `useEscapeKey(clearDraft, draft !== null && !modalOpen)` — gated on exactly the bar's render condition, so PinModal owns Escape once open; checklist reworded to walk both | **R20 — BUILT** |
| **F26** | Re-framing a trip showed an EMPTY title (and year) field — `TripFramePanel` initialises both to `''` unconditionally, so an edit never loaded saved values. Compounded by the placeholder `e.g. "The {destinationName} conference"`, where `destinationName` is `t.title \|\| t.destination_name`: a titled trip has its own title quoted back inside the example, so it reads as ghost text in an empty field | load `defaultTitle` / `defaultYearHint` from the trip, carried separately from `destinationName`. **No data was being lost** — `frame_trip` COALESCEs, so empty meant "unchanged" | **R21 — BUILT** |
