...
**4. Ontology bootstrapping precedes memory collection.** Phase 0 (four-stage ontology bootstrap: temporal skeleton → chapter naming → entity seed → topic map) must be substantially complete before memory collection interviews begin. See `memory/project_lc_ontology_bootstrap.md`.
